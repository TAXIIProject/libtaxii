...
class ContentBlock(BaseNonMessage):
    ...
    def to_etree(self):
        ...
        if self.timestamp_label is not None:
            ...
            tl.text = self.timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
    ...
    def to_dict(self):
        ...
        if self.timestamp_label is not None:
            block['timestamp_label'] = self.timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
...
class PollRequest(TAXIIMessage):
    ...
    def to_etree(self):
        ...
        if self.exclusive_begin_timestamp_label is not None:
            ...
            #TODO: Add TZ Info
            ebt.text = self.exclusive_begin_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
        
        if self.inclusive_end_timestamp_label is not None:
            iet = etree.SubElement(xml, '{%s}Inclusive_End_Timestamp' % ns_map['taxii'])
            #TODO: Add TZ Info
            iet.text = self.inclusive_end_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
    ...
    def to_dict(self):
        ...
        if self.exclusive_begin_timestamp_label is not None:#TODO: Add TZ Info
            d['exclusive_begin_timestamp_label'] = self.exclusive_begin_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
        if self.inclusive_end_timestamp_label is not None:#TODO: Add TZ Info
            d['inclusive_end_timestamp_label'] = self.inclusive_end_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
        d['content_bindings'] = []
...
class PollResponse(TAXIIMessage):
    ...
    def to_etree(self):
        ...
        if self.inclusive_begin_timestamp_label is not None:
            ...
            ibt.text = self.inclusive_begin_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
    ...
    def to_dict(self):
        ...
        if self.message is not None:
            d['message'] = self.message
        if self.inclusive_begin_timestamp_label is not None:
            d['inclusive_begin_timestamp_label'] = self.inclusive_begin_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
        d['inclusive_end_timestamp_label'] = self.inclusive_end_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
        d['content_blocks'] = []
        for block in self.content_blocks:
...
class InboxMessage(TAXIIMessage):
    ...
    class SubscriptionInformation(BaseNonMessage):
        ...
        def to_etree(self):
            ...
            ibtl.text = self.inclusive_begin_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
            
            ietl = etree.SubElement(xml, '{%s}Inclusive_End_Timestamp' % ns_map['taxii'])
            ietl.text = self.inclusive_end_timestamp_label.strftime('%Y-%m-%dT%H:%M:%S.%f')
