...
query1 = tdq.DefaultQuery(t.CB_STIX_XML_11, tdq.DefaultQuery.Criteria())

#Poll Params w/ query and delivery params
poll_params1 = tm11.PollParameters(allow_asynch = True,#Optional - True or False. Defaults to False
                                   response_type = tm11.RT_FULL,#Optional - RT_FULL or RT_COUNT. Defaults to RT_FULL
                                   content_bindings = [tm11.ContentBinding(t.CB_STIX_XML_101), tm11.ContentBinding(t.CB_STIX_XML_11)],#Optional - indicates acceptable content binindings and subtypes
                                   query = None,#Optional - describes a query
                                   delivery_parameters = None)#Optional - describes the delivery parameters

#Poll Params w/o query or delivery params
poll_params2 = tm11.PollParameters(allow_asynch = True,#Optional - True or False. Defaults to False
                                   response_type = tm11.RT_FULL,#Optional - RT_FULL or RT_COUNT. Defaults to RT_FULL
                                   content_bindings = [tm11.ContentBinding(t.CB_STIX_XML_101), tm11.ContentBinding(t.CB_STIX_XML_11)],
                                   query = None,
                                   delivery_parameters = None)

#Poll request to a data feed, and has timestamp labels
poll_request1 = tm11.PollRequest(
        ...
        poll_parameters=poll_params1)  # Optional - one of PollParms or subscription id

message_tests(poll_request1)

#Poll request to a data set, and has timestamp labels
poll_request2 = tm11.PollRequest(
        message_id=tm11.generate_message_id(),  # Required
        collection_name='TheCollectionToPoll',  # Required
        poll_parameters=poll_params1)  # Optional - one of PollParams or subscription ID
        
message_tests(poll_request2)

#Poll request with empty poll params
poll_request2a = tm11.PollRequest(
        message_id=tm11.generate_message_id(),  # Required
        collection_name='TheCollectionToPoll',  # Required
        poll_parameters=tm11.PollParameters())  # Optional - one of PollParams or subscription id
        

#Poll request to a data feed w/ subscription id
poll_request3 = tm11.PollRequest(
        message_id=tm11.generate_message_id(),  # Required
        collection_name='TheCollectionToPoll',  # Required
        exclusive_begin_timestamp_label=datetime.datetime.now(tzutc()),  # Optional - Absence means 'no lower bound'
        inclusive_end_timestamp_label=datetime.datetime.now(tzutc()),  # Optional - Absence means 'no upper bound'
        subscription_id='SubsId001')  # Optional - defaults to accepting all content bindings
message_tests(poll_request3)

#Poll request to a data set w/ subscription id
poll_request4 = tm11.PollRequest(
        message_id=tm11.generate_message_id(),  # Required
        collection_name='TheCollectionToPoll',  # Required
        subscription_id='SubsId001')  # Optional - defaults to accepting all content bindings
message_tests(poll_request4)
...
xml_content_block1 = tm11.ContentBlock(
        content_binding=tm11.ContentBinding(t.CB_STIX_XML_10),  # Required
...
string_content_block1 = tm11.ContentBlock(
        content_binding=tm11.ContentBinding('string'),  # Required
...
status_message1 = tm11.StatusMessage(
        ...
        status_type=tm11.ST_SUCCESS,  # Required
        status_detail={'ThirdPartyName': 'Machine-processable info here!'},  # May be require certain keys, depending on status type
        message='This is a message.')  # Optional
