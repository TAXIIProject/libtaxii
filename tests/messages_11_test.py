...
def message_tests(taxii_message):
    ...
    if valid is not True:
        print 'Bad XML was: %s' % xml_string
...
tdq1 = tdq.DefaultQueryInformation(tdq.FID_TAXII_DEFAULT_QUERY_10, [tdq.TEV_STIX_11], [tdq.CM_CORE])

service_instance1 = tm11.DiscoveryResponse.ServiceInstance(
        ...
        #inbox_service_accepted_content=[tm11.ContentBinding(t.CB_STIX_XML_10)],  # Optional for service_type=SVC_INBOX, prohibited otherwise
        ...
        supported_query=[tdq1])  # Optional for service_type=POLL, COLLECTION_MANAGEMENT. Prohibited otherwise
                                            #If this is absent and service_type = POLL or COLLECTION_MANAGEMENT,
                                            #It means that query is not supported

service_instance2 = tm11.DiscoveryResponse.ServiceInstance(
        service_type=tm11.SVC_INBOX,  # Required
        services_version=t.VID_TAXII_SERVICES_10,  # Required
        protocol_binding=t.VID_TAXII_HTTP_10,  # Required
        service_address='http://example.com/inbox/',  # Required
        message_bindings=[t.VID_TAXII_XML_10],  # Required, must have at least one value in the list
        inbox_service_accepted_content=[tm11.ContentBinding(t.CB_STIX_XML_10, subtype_ids=['test1','test2'])],  # Optional for service_type=SVC_INBOX, prohibited otherwise
                                            #If this is absent and service_type=SVC_INBOX,
                                            #It means the inbox service accepts all content
        available=True,  # Optional - defaults to None, which means 'Unknown'
        message='This is a message.',
        supported_query=None)  # Optional

#Create the discovery response
discovery_response1 = tm11.DiscoveryResponse(
        message_id='TheFirstIdentifier',  # Required
        in_response_to='TheSecondIdentifier',  # Required. This should be the ID of the corresponding request
        service_instances=[service_instance1, service_instance2])  # Optional.

message_tests(discovery_response1)

##Collection Information Request
collection_information_request1 = tm11.CollectionInformationRequest(
        message_id=tm11.generate_message_id())  # Required

message_tests(collection_information_request1)
...
polling_service1 = tm11.CollectionInformationResponse.CollectionInformation.PollingServiceInstance(
...
subscription_service1 = tm11.CollectionInformationResponse.CollectionInformation.SubscriptionMethod(
...
collection1 = tm11.CollectionInformationResponse.CollectionInformation(
        ...
        collection_description='Description of a collection',  # Required
        supported_contents=[tm11.ContentBinding(t.CB_STIX_XML_10, ['subtype1','subtype2'])],  # Required. List of supported content binding IDs
...
collection_information_response1 = tm11.CollectionInformationResponse(
        message_id=tm11.generate_message_id(),  # Required
        in_response_to=tm11.generate_message_id(),  # Required. This should be the ID of the corresponding request
        collection_informations=[collection1])  # Optional

message_tests(collection_information_response1)

## Poll Request

poll_request1 = tm11.PollRequest(
        message_id=tm11.generate_message_id(),  # Required
        collection_name='TheCollectionToPoll',  # Required
...
xml_content_block1 = tm11.ContentBlock(
...
string_content_block1 = tm11.ContentBlock(
        content_binding='string',  # Required
        content='Sample content',  # Required
        padding='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789',  # Optional
        timestamp_label=datetime.datetime.now(tzutc()))  # Optional

poll_response1 = tm11.PollResponse(
        message_id=tm11.generate_message_id(),  # Required
        in_response_to=tm11.generate_message_id(),  # Required - this should be the ID of the corresponding request
...
poll_response2 = tm11.PollResponse(
        message_id=tm11.generate_message_id(),  # Required
        in_response_to=tm11.generate_message_id(),  # Required - this should be the ID of the corresponding request
        collection_name='CollectionName',  # Required
        inclusive_end_timestamp_label=datetime.datetime.now(tzutc()),  # Required
...
status_message1 = tm11.StatusMessage(
        message_id=tm11.generate_message_id(),  # Required
        in_response_to=tm11.generate_message_id(),  # Required. Should be the ID of the corresponding request
...
subscription_information1 = tm11.InboxMessage.SubscriptionInformation(
...
inbox_message1 = tm11.InboxMessage(
        message_id=tm11.generate_message_id(),  # Required
        message='This is a message.',  # Optional
...
inbox_message2 = tm11.InboxMessage(
        message_id=tm11.generate_message_id(),  # Required
        message='This is a message.',  # Optional
        subscription_information=subscription_information1,  # Optional
        content_blocks=[string_content_block1])  # Optional
...
manage_collection_subscription_request1 = tm11.ManageCollectionSubscriptionRequest(
        message_id=tm11.generate_message_id(),  # Required
        collection_name='SomeCollectionName',   # Required
        action=tm11.ACT_UNSUBSCRIBE,  # Required
        subscription_id='SubsId056',  # Required for unsubscribe, prohibited otherwise
        delivery_parameters=delivery_parameters1)  # Required

message_tests(manage_collection_subscription_request1)
...
subscription_instance1 = tm11.ManageCollectionSubscriptionResponse.SubscriptionInstance(
...
manage_collection_subscription_response1 = tm11.ManageCollectionSubscriptionResponse(
        message_id=tm11.generate_message_id(),  # Required
        in_response_to=tm11.generate_message_id(),  # Required - Should be the ID of the corresponding request
        collection_name='Collection001',  # Required
        message='This is a message',  # Optional
        subscription_instances=[subscription_instance1])  # Required

message_tests(manage_collection_subscription_response1)


### Test some Content Blocks ###
cb1 = tm11.ContentBlock(content_binding=t.CB_STIX_XML_10,
                      content='<stix:STIX_Package xmlns:stix="http://stix.mitre.org/stix-1"/>')
contentblock_tests(cb1)

cb2 = tm11.ContentBlock(content_binding=t.CB_STIX_XML_10,
                      content=StringIO.StringIO('<stix:STIX_Package xmlns:stix="http://stix.mitre.org/stix-1"/>'))
contentblock_tests(cb2)

cb3 = tm11.ContentBlock(content_binding=t.CB_STIX_XML_10,
                      content=etree.parse(StringIO.StringIO('<stix:STIX_Package xmlns:stix="http://stix.mitre.org/stix-1"/>')))
contentblock_tests(cb3)

cb4 = tm11.ContentBlock(content_binding=t.CB_STIX_XML_10,
                      content='<Something thats not XML')
contentblock_tests(cb4)

cb5 = tm11.ContentBlock(content_binding=t.CB_STIX_XML_10,
