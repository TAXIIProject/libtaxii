...
service_instance1 = tm.DiscoveryResponse.ServiceInstance(
        service_type=tm.SVC_INBOX,  # Required
...
push_method1 = tm.FeedInformationResponse.FeedInformation.PushMethod(
        push_protocol=t.VID_TAXII_HTTP_10,  # Required
        push_message_bindings=[t.VID_TAXII_XML_10])  # Required

polling_service1 = tm.FeedInformationResponse.FeedInformation.PollingServiceInstance(
...
subscription_service1 = tm.FeedInformationResponse.FeedInformation.SubscriptionMethod(
...
feed1 = tm.FeedInformationResponse.FeedInformation(
...
string_content_block1 = tm.ContentBlock(
...
poll_response2 = tm.PollResponse(
        message_id=tm.generate_message_id(),  # Required
        in_response_to=tm.generate_message_id(),  # Required - this should be the ID of the corresponding request
        feed_name='FeedName',  # Required
        inclusive_end_timestamp_label=datetime.datetime.now(tzutc()),  # Required
...
delivery_parameters1 = tm.DeliveryParameters(
...
poll_instance1 = tm.ManageFeedSubscriptionResponse.PollInstance(
        poll_protocol=t.VID_TAXII_HTTP_10,  # Required
        poll_address='http://example.com/poll',  # Required
        poll_message_bindings=[t.VID_TAXII_XML_10])  # Required

subscription_instance1 = tm.ManageFeedSubscriptionResponse.SubscriptionInstance(
